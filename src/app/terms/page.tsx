...
const sections = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      "ユーザーは、設定画面からいつでもアカウントを削除することができます。アカウント削除時には、以下のデータが完全に削除され、復元はできません：プロフィール情報、保存したコスメデータ・成分情報、スキンケアルーティンの構成データ、成分図鑑の収集データ、スキャン履歴・成分プロファイルデータ、アップロードしたコスメ画像、X連携トークン、端末に保存されたローカルデータ。",
  ...
  },
  {
    ...
    items: [
      ...
      "Myコスメ（スキャンした製品の写真管理・お気に入り・カテゴリ管理）",
      "スキャン履歴・成分プロファイルに基づくパーソナライズド商品レコメンド（楽天市場アフィリエイト連携）",
      "X（旧Twitter）へのコスメ情報シェア機能",
    ],
    paragraphsAfter: [
      "無料アカウントにつきスキャン回数は30回までとなります。スキャン回数はメールアドレス単位で管理され、アカウントを削除して再登録した場合でもリセットされません。機能や利用条件は今後変更される場合があります。",
    ],
  },
  {
    title: "第5条（禁止事項）",
