...
const sections = [
  ...
  },
  {
    ...
    subsections: [
      ...
      },
      {
        ...
        items: [
          "提供する情報：OAuth認証に必要な情報、スキャンした化粧品のパッケージ画像・成分表示の画像",
          "利用目的：Googleアカウントによるログイン認証、Google Gemini APIによるAI画像認識・成分名抽出（パッケージからの製品特定および成分表示テキストの抽出）・成分検索",
          "※画像はAI処理のために送信され、保存されません。Googleの利用規約・プライバシーポリシーに従いデータが取り扱われます。",
        ],
      },
      {
      ...
      {
        subtitle: "（5）Google LLC（米国）— Google Analytics",
        items: [
  ...
  },
  {
    ...
    content: `本サービスでは、上記第4条に記載のとおり、米国に所在する事業者が提供するクラウドサービスを利用しています。これにより、ユーザーの個人データが日本国外で取り扱われる場合があります。各事業者は、それぞれのプライバシーポリシーに基づき適切なデータ保護措置を講じています。`,
  },
  {
    title: "6. 安全管理措置",
