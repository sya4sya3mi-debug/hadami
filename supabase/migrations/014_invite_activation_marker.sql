...
-- Backfill legacy users (before invite-enforcement start) so existing users keep access.
update public.profiles p
set invite_activated_at = coalesce(p.invite_activated_at, p.created_at)
from auth.users u
where p.id = u.id
  and nullif(btrim(p.display_name), '') is not null
  and u.created_at < timestamp with time zone '2026-04-22T00:00:00.000Z';
...
begin
  ...
  if v_profile_exists then
    v_has_completed_profile :=
      ...
      and v_profile.invite_activated_at is not null;
  ...
  if v_profile_exists then
    update public.profiles
    set
      display_name = v_trimmed_display_name,
      invite_activated_at = coalesce(invite_activated_at, now())
    where id = v_user_id;
  else
    insert into public.profiles (id, display_name, invite_activated_at)
    values (v_user_id, v_trimmed_display_name, now());
  end if;
